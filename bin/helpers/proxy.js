...
class ProxyHelper {
     ...
     */
    async checkProxy() {
        if (mz_1.fs.existsSync('../data/bad.proxies.json')) {
            // we put all bad proxy in a file, and keep them for 5 days
            const loaded = await mz_1.fs.readFile('../data/bad.proxies.json', 'utf8');
            this.badProxies = JSON.parse(loaded);
            this.badProxies = _.filter(this.badProxies, p => moment(p.date).isAfter(moment().subtract(5, 'day')));
            await mz_1.fs.writeFile('../data/bad.proxies.json', JSON.stringify(this.badProxies, null, 2));
        }
